...
class Game():
    ...
    def step(self,action):
        ...
        if not self.is_valid_action(action):
            ...
            # Add a line where the action dictates
            self.build_line(action)
    ...
    def score_action(self,action):
        ...
        :param action: The integer representing the line to be built
    ...
    def play(self, log=False):
        ...
        while self.state is not None:
            if log:
                ...
                game_log.append("{} built line {}".format(acting_player, action))
    ...
    def build_line(self,action):
        """Builds a line in the game state"""
        states = self.convert_to_state(action)
        for state_index in states:
            row,column,side = state_index
            self.state[row,column,side] = 1

    def convert_to_line(self,state):
        """Converts a specific game state array to the line it represents"""
        ...
        if side == self.SIDES['N']:
            ...
            raise ValueError("Can't convert state to line. 3rd Dimension must be range [0-3]. Value given: {}".format(side))

    def convert_to_state(self,line_number):
        """Converts a line number to the specific game states that it represents"""
        cells = []
        #If this is true, the line is on the N-S
        if line_number < ((self.size) * (self.size + 1)):
            cell_column = line_number % self.size
            cell_row = line_number // self.size
            if cell_row != 0:
                cells.append([cell_row - 1, cell_column, self.SIDES['S']])
            if cell_row != self.size:
                cells.append([cell_row, cell_column, self.SIDES['N']])
        #Otherwise the line is E-W
        else:
            cell_row = (line_number-(self.size * (self.size + 1)))//(self.size + 1)
            cell_column = line_number % (self.size + 1)
            if cell_column != 0:
